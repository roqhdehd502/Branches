			...
			<div class="row">
				...
				<span style="border-left: 1px solid rgba(0, 0, 0, .1); width: 922px;">
					...
					<div class="team-area sp">
						<div class="container">
							<div class="row">
								<form id="searchForm" action="/seller/mypage/cancel" method="get" style="position: relative; left: 480px; bottom: 20px;">
									<span>
								...
								<table class="table">
									...
									<tbody>
										<c:forEach items="${cancel }" var="prdct" varStatus="status">
											<tr style="text-align: center;">
												...
												<td>
													<c:if test="${prdct.order_state_number eq 8}">
														<h5 style="position: relative; top: 20px;">주문취소</h5>
													</c:if>
								...
								<ul class="pagination" style="margin: auto;">
									<c:if test="${pageMaker.prev}">
										<a class="page-link" href="/seller/mypage/cancel${pageMaker.makeQuery(pageMaker.startPage - 1)}">«</a>
									</c:if>

									<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
										<c:out value="${pageMaker.cri.pageNum == idx?'':''}" />
										<a class="page-link" href="/seller/mypage/cancel${pageMaker.makeQuery(idx)}">${idx}</a>
									</c:forEach>

									<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
										<a class="page-link" href="/seller/mypage/cancel${pageMaker.makeQuery(pageMaker.endPage +1)}">»</a>
									</c:if>
								</ul>
							</div>
						</div>
