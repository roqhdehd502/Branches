			...
			<div class="row">
				...
				<span style="border-left: 1px solid rgba(0, 0, 0, .1); width: 922px;">
					<h3 style="margin-top: 5px; margin-left: 15px; padding-bottom: 16px;">
						<strong style="margin: 10px;">교환 확인</strong>
					</h3>
					<div class="team-area sp">
						<div class="container">
							<div class="row">
								<form id="searchForm" action="/seller/mypage/exchange" method="get" style="position: relative; left: 480px; bottom: 20px;">
									<span>
								...
								<table class="table">
									...
									<tbody style="text-align: center;">
										<c:forEach items="${exchange }" var="prdct" varStatus="status">
											<tr>
												<td>
												...
												<td>
													<c:if test="${prdct.order_state_number eq 9}">
														<h5 style="position: relative; top: 20px;">교환요청</h5>
													</c:if>
								...
								<ul class="pagination" style="margin: auto;">
									<c:if test="${pageMaker.prev}">
										<a class="page-link" href="/seller/mypage/exchange${pageMaker.makeQuery(pageMaker.startPage - 1)}">«</a>
									</c:if>

									<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
										<c:out value="${pageMaker.cri.pageNum == idx?'':''}" />
										<a class="page-link" href="/seller/mypage/exchange${pageMaker.makeQuery(idx)}">${idx}</a>
									</c:forEach>

									<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
										<a class="page-link" href="/seller/mypage/exchange${pageMaker.makeQuery(pageMaker.endPage +1)}">»</a>
									</c:if>
								</ul>
							</div>
						</div>
